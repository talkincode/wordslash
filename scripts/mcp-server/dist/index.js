...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        switch (name) {
            case 'list_cards': {
                const { search, tag, limit = 50 } = (args || {});
                const cards = await storage.getCards();
                let result = Array.from(cards.values());
                // Apply filters
                if (search) {
                    const searchLower = search.toLowerCase();
                    result = result.filter((c) => c.front.term.toLowerCase().includes(searchLower) ||
            ...
            case 'delete_cards_batch': {
                const { ids, search, tag, confirm } = (args || {});
                if (!confirm) {
                    return {
            ...
            case 'rebuild_index': {
                ...
                return {
                    content: [
                        {
                            type: 'text',
                            text: `Index rebuilt successfully:\n${JSON.stringify({ ...index, updatedAt: new Date(index.updatedAt).toISOString() }, null, 2)}`,
                        },
                    ],
                };
            }
            ...
            case 'generate_knowledge_graph': {
                const { maxNodes = 100, includeOrphans = false, tag } = (args || {});
 ...
 */
function calculateReviewsPerDay(events, days) {
    const result = [];
 ...
 */
async function generateKnowledgeGraph(maxNodes, includeOrphans, tag) {
    const cards = await storage.getCards();
    const events = await storage.readAllEvents();
    ...
    for (const card of filteredCards) {
        let connections = 0;
        if (card.back?.synonyms?.length)
            connections += card.back.synonyms.length;
        if (card.back?.antonyms?.length)
            connections += card.back.antonyms.length;
        if (card.tags?.length)
            connections += card.tags.length;
    ...
    const getRelatedTermId = (term, type) => {
    ...
    for (const card of filteredCards) {
        ...
        if (card.back?.synonyms) {
            for (const synonym of card.back.synonyms) {
                ...
                const matchingCard = filteredCards.find(c => c.id !== card.id && c.front.term.toLowerCase() === synonym.toLowerCase());
                if (matchingCard) {
        ...
        if (card.back?.antonyms) {
            for (const antonym of card.back.antonyms) {
                ...
                const matchingCard = filteredCards.find(c => c.id !== card.id && c.front.term.toLowerCase() === antonym.toLowerCase());
                if (matchingCard) {
                    edges.push({
                        source: card.id,
                        target: matchingCard.id,
