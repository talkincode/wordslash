...
export class Storage {
     ...
     */
    async readAllCardEntries() {
        return this.readJsonl(CARDS_FILE);
    }
    /**
     ...
     */
    async readAllEvents() {
        return this.readJsonl(EVENTS_FILE);
    }
    /**
     * Get events for a specific card
     */
    async getEventsForCard(cardId) {
     ...
     */
    async readIndex() {
        return this.readJson(INDEX_FILE);
    }
    /**
     ...
     */
    async atomicWriteJson(filename, data) {
        await this.ensureDir();
        const filePath = path.join(this.basePath, filename);
